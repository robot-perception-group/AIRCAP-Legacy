...
int replay::run(void)
{
    ...
    unsigned long long pts,vpts, time, starttime;
    ...
    while (ros::ok()) {
	// read frame
	ret = libavhelper_read_frame(framebuffer,&vpts);
	...
	instance->frame_pub.publish(msg);
		...
		rosinfoPrint( static_cast<std::ostringstream*>( &(std::ostringstream() <<  "Frame: " << frames << "/" << pts << "(" << vpts << "), Average rate: " << (double)(1000000.*pts)/diff.total_microseconds() << ", (" << (double)(1000000.*frames)/diff.total_microseconds() << "), Skipped frames: " << (pts-frames) << std::endl ))->str().c_str());
	//} 
