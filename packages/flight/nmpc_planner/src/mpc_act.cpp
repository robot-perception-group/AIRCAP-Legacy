...
void Planner::avoidTeamMates_byComputingExtForce()
{
  ...
  for(int t=0;t<16;t++)
  {
    ...
    potentialForce_repulsive = getPotential(targetDist,distanceThresholdToTarget,distanceThresholdToTarget-1,50); // away from target
    ...
    for(int j=0; j<=numRobots_-1; j++)
    {
      if(heardFromMates[j])
      {
        ...
        double obstacleAvoidanceThreshold = neighborDistThreshold + maxOffsetUncertaintyRadius + covariance_radius + comm_failure_dilation;
        ROS_INFO("obstacleAvoidanceThreshold : %f", obstacleAvoidanceThreshold);
        potentialForce_repulsive = getPotential(neighborDist,obstacleAvoidanceThreshold,1,50); //away from neighbor
        ...
        potentialForce_angular = getPotential(thetaDiff,pow((2*PI/numRobots_)+0.8,2),0,50);
        ...
        if (posDiffUnit.dot(tangentUnit) > posDiffUnit.dot(tangentUnitNeg))
          totalForce += force_clamping((potentialForce_angular) * tangentUnit *  abs(targetDist - r) );
          // totalForce += (potentialForce_angular) * tangentUnit *  0;
        else
          totalForce += force_clamping((potentialForce_angular) * tangentUnitNeg *  abs(targetDist - r)) ;
    ...
    if  (POINT_OBSTACLES)
    {
      ...
      for (int i=0; i < obstacles_length; i++)
      {
        ...
        double neighborDist = posDiff.norm();
        Position3D posDiffUnit = posDiff.normalized();
        potentialForce_repulsive = getPotential(neighborDist,staticObsDistThreshold+covariance_radius,1,50);
        totalForce += force_clamping((potentialForce_repulsive) * posDiffUnit );
        potentialForce_repulsive = force_clamping((potentialForce_repulsive) * posDiffUnit ).norm();
        if (neighborDist <= 3)
        {
          double thetaMate = atan2(matePosition(1)-yT,matePosition(0)-xT) ? atan2(matePosition(1)-yT,matePosition(0)-xT) : 2*PI + atan2(matePosition(1)-yT,matePosition(0)-xT);
          double theta = atan2(tCurrentSelfPosition(1)-yT,tCurrentSelfPosition(0)-xT) ? atan2(tCurrentSelfPosition(1)-yT,tCurrentSelfPosition(0)-xT) : 2*PI + atan2(tCurrentSelfPosition(1)-yT,tCurrentSelfPosition(0)-xT);
         ...
         // ROS_INFO("thetaDiff: %f, mateID:%d", temp*180/PI,j);
         potentialForce_angular = getPotential(thetaDiff,pow(0.5,2),0,50);
         ...
         ROS_INFO("force: %f", potentialForce_repulsive);
      }
    }
...
void Planner::reconf_callback(nmpc_planner::nmpcPlannerParamsConfig &config) //R:Dynamic Reconfigure?
{
  ...
  ROS_INFO("Reconfigure Request: staticObsDistThreshold = %f ",config.staticObsDistThreshold);

  neighborDistThreshold=config.neighborDistThreshold;

  staticObsDistThreshold = config.staticObsDistThreshold;
